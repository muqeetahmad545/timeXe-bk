...
export const createUser = async (
  ...
  try {
    ...
    const hashedConfirmPassword = await bcrypt.hash(signInDetail.confirmPassword, 10);
...
export const authUser = async (req: Request, res: Response) => {
  const { signInEmail, password } = req.body; // Change to signInEmail

  try {
    // Check if the user exists by signInEmail
